...
public class ResultServlet extends HttpServlet {
	@Override
	protected void doPost( //右クリックフォーマットで生成できるんかい..。
			HttpServletRequest request,
			HttpServletResponse response
			)throws ServletException, IOException{
			request.setCharacterEncoding("UTF-8");//ユーザーが送信したパラメーターの文字コード
			...
			String jsp = null;
			if(name == null || name.isEmpty()) {//nameが空なら結果を表示しない
				request.setAttribute("message", "名前を入力しろや...。");
